...
static const struct regval ov13855_global_regs[] = {
	...
	{0x5407, 0x01},
	{0x5408, 0x4a},
	{0x3503, 0x08},//关闭自动曝光增益，备份增益使能位设置�?3
	{REG_NULL, 0x00},
};
...
/*
 * Xclk 24Mhz
 * max_framerate ~60fps (VTS=1200, adjustable via vblank for longer exposure)
 * mipi_datarate per lane 1080Mbps
 * 1920x1080 center crop from full sensor array, no binning/subsample
 * RAW BAYER SBGGR10 output
 */
static const struct regval ov13855_1920x1080_regs[] = {
	//配置参�? ov13855_4224x3136_30fps_regs[] 
	{0x0300, 0x02},
	{0x0301, 0x00},
	{0x0302, 0x5a},
		{0x0303, 0x01},		// PLL1_DIV_MIPI = /2 (link_freq=270MHz)
	{0x0304, 0x00},
	{0x0305, 0x01},
	{0x030b, 0x06},
	{0x030c, 0x02},
	...
	{0x3106, 0x15},
	{0x3107, 0x23},
		//默认曝光行数 1024 (VTS=3209, VTS-4=3205)
	{0x3500, 0x00},
		{0x3501, 0x40},		// exposure=0x0400=1024 lines (VTS-4=3205)
		{0x3502, 0x00},		// group hold
	//默认模拟增益数字增益
	{0x3508, 0x02},
	{0x3509, 0x00},
	{0x350a, 0x00},
	...
	{0x37e5, 0x03},
	{0x37e6, 0x04},
	//从传感器全幅像素阵列中裁剪出1920*1080的窗口（crop模式�?
	//x_start=(4256 - 1920) / 2 = 1168, y_start= (3152 - 1080) / 2 + 8 = 1044, x_end=3087, y_end=2123
	//坐标偶数对齐，若从奇数开始裁，bayer阵列会变
	{0x3800, 0x04},  
	{0x3801, 0x90},  //x_start�?0x0490
	{0x3802, 0x04}, 
	{0x3803, 0x14},  //y_start�?0x0414
	{0x3804, 0x0c},  
	{0x3805, 0x0f},  //x_end�?0x0c0f
	{0x3806, 0x08},  
	{0x3807, 0x4b},  //y_end�?0x084b
	{0x3808, 0x07}, 
	{0x3809, 0x80},  //output_width�?0x0780
	{0x380a, 0x04},  
	{0x380b, 0x38},  //output_height�?0x0438
		//HTS = 1122 (matching debug_60fps)
		{0x380c, 0x04},		// HTS = 0x0462 = 1122
		{0x380d, 0x62},
		//VTS = 3209 (matching debug_60fps)
		{0x380e, 0x0c},		// VTS = 0x0C89 = 3209
		{0x380f, 0x89},
	//以下参考ov13855_1920x1080_regs[]设置
	{0x0300, 0x02},
	{0x0301, 0x00},
	{0x0302, 0x5a},
	{0x0303, 0x00},
	...
	{0x37e6, 0x04},
	{0x3800, 0x04},
	{0x3801, 0x90},  //x_start=0x0490=1168
	{0x3802, 0x04},
	{0x3803, 0x14},  //y_start=0x0414=1044
	{0x3804, 0x0c},
	{0x3805, 0x0f},  //x_end=0x0c0f=3087
	{0x3806, 0x08},
	{0x3807, 0x4b},  //y_end=0x084b=2123
	{0x3808, 0x07},
	{0x3809, 0x80},  //output_width=0x0780=1920
	{0x380a, 0x04},
	{0x380b, 0x38},  //output_height=0x0438=1080
	{0x380c, 0x04},
	{0x380d, 0x62},
	{0x380e, 0x0c},
	{0x380f, 0x8e},
...
static const struct ov13855_mode supported_modes[] = {
	...
	// 1920x1080 crop模式
	{
		...
		.max_fps = {
			...
			.exp_def = 0x0800,	//2048 (same as 30fps, VTS=3214)

			.hts_def = 0x0462,	//1122 (match debug_60fps timing)
			.vts_def = 0x0c8e,	//3214 (same as 30fps mode)

		.bpp = 10,
		.reg_list = ov13855_1920x1080_regs,
			.link_freq_idx = 0,	// 540MHz MIPI (same as 30fps mode)
...
static const struct ov13855_mode *ov13855_find_mode(struct ov13855 *ov13855, int fps)
{
	const struct ov13855_mode *mode = NULL;
	const struct ov13855_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == ov13855->cur_mode->width &&
		    mode->height == ov13855->cur_mode->height &&
		    mode->bus_fmt == ov13855->cur_mode->bus_fmt) {
...
static int ov13855_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct ov13855 *ov13855 = to_ov13855(sd);
	const struct ov13855_mode *mode = NULL;
	...
	mode = ov13855_find_mode(ov13855, fps);
	if (mode == NULL) {
...
static const struct v4l2_subdev_video_ops ov13855_video_ops = {
	...
	.s_frame_interval = ov13855_s_frame_interval,
};
...
static void ov13855_modify_fps_info(struct ov13855 *ov13855)
{
	const struct ov13855_mode *mode = ov13855->cur_mode;

	ov13855->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      ov13855->cur_vts;
}
...
static int ov13855_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		ov13855->cur_vts = ctrl->val + ov13855->cur_mode->height;
		ov13855_modify_fps_info(ov13855);
		break;
