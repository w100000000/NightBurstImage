...
class UnifiedTrainer(TrainingModule):
    ...
    与 TwoPhaseTrainer 的区别：
    - 不需要 deblurNet（无两阶段）
    - 不需要 downsample_tensors()
    - 输入 3 帧 RAW 而非 2 帧 RGB
    - 数据集用 RAW3RGB_dataset
    """
    ...
    def train(self):
        ...
        for epoch in range(self.Training_config.start_idx + 1, self.Training_config.epochs):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                # ============= Get data ==============
                short1 = data['short1_raw'].to(self.device)    # [B, 4, H, W]
                long = data['long_raw'].to(self.device)         # [B, 4, H, W]
                short2 = data['short2_raw'].to(self.device)     # [B, 4, H, W]
                gt = data['RGBout_img'].to(self.device)         # [B, 3, H, W]

                # ============== forward ===============
                out = self.G(short1, long, short2)              # [B, 3, H, W]
                ...
                if iters_done % self.Training_config.show_loss_iter == 0:
                    self.add_scalars(main_tag='G_loss', tag_scalar_dict=G_loss_info, global_step=iters_done)
                if iters_done % self.Training_config.show_img_iter == 0:
                    vis_imgs = [long, gt, out.clamp(0.0, 1.0)]
                    self.visual_image('train_img', vis_imgs, iters_done)
