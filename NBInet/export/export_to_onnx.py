...
说明:
    - RAW 输入需被 8 整除（编码器 3 级 stride-2 下采样）
    - 1080p RGB (1920x1080) → RAW RGGB 各平面的分辨率: 960x540
    - 540 不能被 8 整除，取 544 → 输出 1088×1920 → crop 到 1080×1920
...
def export_to_onnx(model_path, output_path, height=544, width=960, opset=12):
    """
    ...
    参数:
        ...
        height: RAW 输入高度（1080p→544, 必须被8整除）
    ...
    print('  Output:              [1, 3, %d, %d] (RGB; crop to 1080x1920)' % (height * 2, width * 2))
